...
const BASE = '/admin/riwayat-jurnal/tatausaha';

export default function AdminRiwayatJurnalTU({ riwayat, staffList, filters }) {
    ...
    return (
        <AppLayout title="Riwayat Jurnal Tata Usaha">
            <div className="space-y-5">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <History className="h-5 w-5 text-sky-600" /> Riwayat Jurnal Tata Usaha
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Rekap jurnal seluruh karyawan tata usaha</p>
                </div>
                        ...
                        <div className="flex flex-wrap items-end gap-3">
                            <div className="min-w-52">
                                <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                                    <Users className="inline h-3 w-3 mr-1" />Karyawan
                                </label>
                                    ...
                                    {riwayat.data.map(item => (
                                        <div key={item.id} className="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                            <div className="flex items-start justify-between gap-2 mb-1">
                                                <p className="font-semibold text-sm text-gray-800 dark:text-gray-200">{item.tatausaha?.user?.name ?? '—'}</p>
                                                <p className="text-xs text-gray-400 shrink-0">{fmtShort(item.tanggal)}</p>
                                            </div>
                                            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                                                <span className="text-xs text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-900/30 px-1.5 py-0.5 rounded">{item.tatausaha?.jabatan ?? '—'}</span>
