...
const THEME = {
    'Wakasek':         { ring: 'focus:ring-sky-500',    badge: 'bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400',    accent: 'text-sky-600' },
    'Kepala Tata Usaha': { ring: 'focus:ring-teal-500', badge: 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400', accent: 'text-teal-600' },
};

export default function AdminRiwayatJurnalPimpinan({ riwayat, staffList, filters, title, basePath }) {
    const theme = THEME[title] ?? THEME['Wakasek'];
    ...
    return (
        <AppLayout title={`Riwayat Jurnal ${title}`}>
            <div className="space-y-5">
                <div>
                    <h1 className={`text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2`}>
                        <History className={`h-5 w-5 ${theme.accent}`} /> Riwayat Jurnal {title}
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                        Rekap jurnal {title === 'Wakasek' ? 'wakil kepala sekolah' : 'kepala tata usaha'}
                    </p>
                </div>
                ...
                <Card>
                    <CardBody className="p-4">
                        <div className="flex flex-wrap items-end gap-3">
                            <div className="min-w-52">
                                <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                                    <Users className="inline h-3 w-3 mr-1" />{title}
                                </label>
                                <select value={staff} onChange={e => hStaff(e.target.value)}
                                    className={`w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 ${theme.ring}`}>
                            ...
                            <div>
                                ...
                                <input type="date" value={dari} onChange={e => hDari(e.target.value)}
                                    className={`rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 ${theme.ring}`} />
                            </div>
                            <div>
                            ...
                            <div className="flex-1 min-w-44">
                                ...
                                <div className="relative">
                                    ...
                                    <input type="text" value={q} onChange={e => hQ(e.target.value)}
                                        ...
                                        className={`w-full pl-8 pr-3 py-1.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 ${theme.ring}`} />
                                </div>
                            </div>
                            {hasFilter && (
                        ...
                        {selectedStaff && (
                            <div className="mt-2">
                                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${theme.badge}`}>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            {riwayat.total} Entri
                            {hasFilter && <span className={`text-xs font-normal ${theme.accent}`}>(difilter)</span>}
                        </CardTitle>
                    </CardHeader>
                                    ...
                                    {riwayat.data.map(item => (
                                        <div key={item.id} className="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                            <div className="flex items-start justify-between gap-2 mb-1">
                                                <p className="font-semibold text-sm text-gray-800 dark:text-gray-200">{item.user?.name ?? '—'}</p>
                                                <p className="text-xs text-gray-400 shrink-0">{fmtShort(item.tanggal)}</p>
                                            </div>
                                            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                                                <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${theme.badge}`}>{item.jabatan}</span>
