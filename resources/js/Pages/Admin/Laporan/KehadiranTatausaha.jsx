...
import { Users, Filter, TrendingUp, FileSpreadsheet, Printer, X } from 'lucide-react';
...
function pct(hadir, hari) {
    return hari > 0 ? Math.round((hadir / hari) * 100 * 10) / 10 : 0;
}
...
function PrintOverlay({ rekap, hariKerja, period, kop, sekolah, onClose }) {
    ...
    const totSakit = rekap.reduce((s, r) => s + r.sakit, 0);
    const totIzin  = rekap.reduce((s, r) => s + r.izin, 0);
    const totAlpha = rekap.reduce((s, r) => s + r.alpha, 0);
    ...
    return (
        <div className="fixed inset-0 bg-white z-9999 overflow-auto print-overlay">
            ...
            <div className="no-print sticky top-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between z-10 shadow-sm">
                <p className="text-sm font-medium text-gray-700">Preview Print — Kehadiran Tata Usaha</p>
            ...
            <div className="max-w-4xl mx-auto px-8 py-6">
                ...
                <div className="text-center mb-5">
                    <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-1">Rekap Kehadiran Tata Usaha</p>
                ...
                <table className="w-full text-sm border border-gray-300">
                    <thead>
                        <tr style={{ backgroundColor: '#F3F4F6' }}>
                            ...
                            <th className="px-3 py-2 text-center border border-gray-300 text-sky-700">Sakit</th>
                            <th className="px-3 py-2 text-center border border-gray-300 text-amber-700">Izin</th>
                            <th className="px-3 py-2 text-center border border-gray-300 text-red-600">Alpha</th>
                            <th className="px-3 py-2 text-center border border-gray-300">% Hadir</th>
                        </tr>
                    </thead>
                    <tbody>
                        {rekap.map((row, idx) => (
                            <tr key={row.id} style={idx % 2 !== 0 ? { backgroundColor: '#F9FAFB' } : {}}>
                                ...
                                <td className="px-3 py-1.5 text-center text-sky-700 border border-gray-300">{row.sakit}</td>
                                <td className="px-3 py-1.5 text-center text-amber-700 border border-gray-300">{row.izin}</td>
                                <td className="px-3 py-1.5 text-center text-red-600 border border-gray-300">{row.alpha}</td>
                    ...
                    <tfoot>
                        <tr style={{ backgroundColor: '#F3F4F6', fontWeight: 'bold' }}>
                            ...
                            <td className="px-3 py-2 text-center text-sky-700 border border-gray-300">{totSakit}</td>
                            <td className="px-3 py-2 text-center text-amber-700 border border-gray-300">{totIzin}</td>
                            <td className="px-3 py-2 text-center text-red-600 border border-gray-300">{totAlpha}</td>
                ...
                <div className="flex justify-between items-end mt-8">
                    ...
                    <div className="text-center text-sm text-gray-600">
                        ...
                        {sekolah?.nip_kepala && (
                            <p className="text-xs text-gray-500">NIP. {sekolah.nip_kepala}</p>
                        )}
                    </div>
                </div>
            </div>
...
export default function KehadiranTatausaha({ rekap, bulan, hariKerja, kop, sekolah }) {
    ...
    return (
        <AppLayout title="Kehadiran Tata Usaha">
            {showPrint && (
                <PrintOverlay
                    rekap={rekap}
                    hariKerja={hariKerja}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <input
                        type="month"
                        value={bulan}
                        onChange={(e) => router.get('/admin/laporan/kehadiran-tatausaha', { bulan: e.target.value }, { preserveState: true })}
                    ...
                    <a
                        href={`/admin/laporan/kehadiran-tatausaha/export?bulan=${bulan}`}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                {[
                    ...
                    { label: 'Sakit', val: totSakit, cls: 'text-sky-600 dark:text-sky-400' },
                    { label: 'Izin',  val: totIzin,  cls: 'text-amber-600 dark:text-amber-400' },
                    { label: 'Alpha', val: totAlpha, cls: 'text-red-600 dark:text-red-400' },
                ].map(({ label, val, cls }) => (
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-gray-50 dark:bg-gray-900/50 text-xs uppercase text-gray-500">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-center text-sky-600">Sakit</th>
                                        <th className="px-4 py-3 text-center text-amber-600">Izin</th>
                                        <th className="px-4 py-3 text-center text-red-500">Alpha</th>
                                        <th className="px-4 py-3 text-center min-w-[150px]">Kehadiran</th>
                                    </tr>
                                </thead>
                                    ...
                                    {rekap.map((row, idx) => {
                                        ...
                                        return (
                                            <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                                ...
                                                <td className="px-4 py-3 text-center text-sky-600 dark:text-sky-400">{row.sakit}</td>
                                                <td className="px-4 py-3 text-center text-amber-600 dark:text-amber-400">{row.izin}</td>
                                                <td className="px-4 py-3 text-center text-red-500">{row.alpha}</td>
                                ...
                                <tfoot className="bg-gray-50 dark:bg-gray-900/50 border-t-2 border-gray-200 dark:border-gray-700">
                                    <tr>
                                        ...
                                        <td className="px-4 py-3 text-center font-bold text-sky-600">{totSakit}</td>
                                        <td className="px-4 py-3 text-center font-bold text-amber-600">{totIzin}</td>
                                        <td className="px-4 py-3 text-center font-bold text-red-500">{totAlpha}</td>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2">
                                                <div className="flex-1 h-2 rounded-full bg-gray-100 dark:bg-gray-700">
