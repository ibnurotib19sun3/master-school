...
export default function GuruCatatanKepsek({ catatan }) {
        ...
        <AppLayout title="Catatan dari Kepala Sekolah">
            ...
            {items.length === 0 ? (
                <div className="text-center py-20 text-gray-400">
                    ...
                    <p className="text-xs mt-1">Catatan dari kepala sekolah akan muncul di sini.</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {items.map((item) => (
                        <Card
                            key={item.id}
                            className={`border-l-4 ${KATEGORI_BG[item.kategori] ?? 'border-l-gray-300'}`}
                        >
                            <CardBody>
