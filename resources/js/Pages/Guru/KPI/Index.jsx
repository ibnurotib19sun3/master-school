            ...
            <CardBody className="space-y-4">
                ...
                <div className="space-y-3 pt-1 border-t border-gray-100 dark:border-gray-800">
                    <BarRow
                        ...
                        persen={Number(item.persen_guru ?? 0)}
                        bobot={Number(item.bobot_guru ?? 50)}
                        icon={UserCheck}
                        color="text-sky-500"
                    />
                    <BarRow
                    ...
                    />
                    {item.persen_manajemen != null && (
                        <BarRow
                            label="Kehadiran Manajemen"
                            persen={Number(item.persen_manajemen)}
                            bobot={Number(item.bobot_manajemen ?? 0)}
                            icon={Crown}
...
// Ringkasan rata-rata dari semua rekap
function RingkasanCard({ rekap }) {
    if (!rekap.length) return null;
    const rataGuru   = Math.round(rekap.reduce((s, r) => s + Number(r.persen_guru ?? 0), 0) / rekap.length * 10) / 10;
        ...
        <Card>
            ...
            <CardBody>
                <div className="grid grid-cols-3 divide-x divide-gray-100 dark:divide-gray-800 text-center">
                    {[
                        { label: 'Rata-rata Kehadiran', value: `${rataGuru}%`, icon: UserCheck, color: 'text-sky-500' },
...
export default function GuruKPI({ rekap = [] }) {
    return (
                ...
                {rekap.length === 0 ? (
                    <Card>
                        <CardBody className="py-16 text-center">
                            ...
                            <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">KPI akan muncul di sini setelah dinilai oleh Kepala Sekolah atau Kurikulum.</p>
                        </CardBody>
                    </Card>
                ) : (
                    <>
