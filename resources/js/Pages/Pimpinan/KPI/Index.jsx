...
import { TrendingUp, UserCheck, BookOpen, Star, CheckCircle2, AlertCircle, CalendarDays, Crown } from 'lucide-react';
            ...
            <CardBody className="space-y-4">
                ...
                <div className="space-y-3 pt-1 border-t border-gray-100 dark:border-gray-800">
                    <BarRow
                        ...
                        persen={Number(item.persen_hadir ?? 0)}
                        bobot={Number(item.bobot_hadir ?? 50)}
                        icon={UserCheck}
                        color="text-amber-500"
                    />
                    <BarRow
                        label="Keaktifan Jurnal Pimpinan"
                ...
                <div className="flex items-start gap-2 pt-1 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                        {item.catatan
                            ? item.catatan
                            : isBaik
                                ? 'KPI bulan ini memenuhi target.'
                                : 'KPI bulan ini belum memenuhi target 80.'}
                    </p>
                </div>
            </CardBody>
        </Card>
...
function RingkasanCard({ rekap }) {
    ...
    const rataHadir  = Math.round(rekap.reduce((s, r) => s + Number(r.persen_hadir  ?? 0), 0) / rekap.length * 10) / 10;
        ...
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2">
                    <Star className="h-5 w-5 text-amber-500" /> Ringkasan Keseluruhan
                </CardTitle>
            </CardHeader>
            <CardBody>
                <div className="grid grid-cols-3 divide-x divide-gray-100 dark:divide-gray-800 text-center">
                    {[
                        { label: 'Rata-rata Kehadiran',  value: `${rataHadir}%`,  icon: UserCheck,  color: 'text-amber-500' },
                        { label: 'Rata-rata Jurnal',     value: `${rataJurnal}%`, icon: BookOpen,   color: 'text-emerald-500' },
                        { label: 'Rata-rata Nilai KPI',  value: rataNilai,        icon: TrendingUp, color: 'text-sky-500' },
...
export default function PimpinanKPI({ rekap = [], jabatan = 'Pimpinan' }) {
    return (
        <AppLayout title="Laporan KPI Saya">
            <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    Laporan kinerja kehadiran dan jurnal pimpinan
                </p>
            </div>
                    ...
                    <Card>
                        <CardBody className="py-16 text-center">
                            ...
                            <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                                KPI akan muncul di sini setelah dinilai oleh admin.
                            </p>
                        </CardBody>
                    </Card>
                ) : (
